...
# Builder metadata
IMAGE_VENDOR  ?= Ahmad Ainul Rizki<human@shitiomatic.email>
...
ifeq ($(UPSTREAM_PRESENT),true)
	UPSTREAM_ARGS :=  --label io.github.shitiomatic.upstream.author="$(UPSTREAM_AUTHOR)" \
    --label io.github.shitiomatic.upstream.url="$(UPSTREAM_URL)"
else
	UPSTREAM_ARGS :=
endif
...
docker: ## Build docker image.
    ...
    --label io.github.shitiomatic.build.system="$(IMAGE_BUILD_SYSTEM)" \
    --label io.github.shitiomatic.build.host="$(IMAGE_BUILD_HOST)" \
    --label io.github.shitiomatic.build.date="$(IMAGE_BUILD_DATE)" \
    --label io.github.shitiomatic.actions.workflow="$(GITHUB_WORKFLOW)" \
    --label io.github.shitiomatic.actions.build="$(GITHUB_RUN_NUMBER)" \
    --label io.github.shitiomatic.actions.actor="$(GITHUB_ACTOR)" \
    --label io.github.shitiomatic.actions.ref="$(GITHUB_REF)" \
    --label io.github.shitiomatic.git.commit="$(GIT_COMMIT)" \
    --label io.github.shitiomatic.upstream.present="$(UPSTREAM_PRESENT)" \
    $(UPSTREAM_ARGS) \
